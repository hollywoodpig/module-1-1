...
class Ezfc_Element_Numbers extends Ezfc_Element {
	public function get_output() {
		...
		$el_text .= "<input	class='{$this->data->class} ezfc-element ezfc-element-numbers ezfc-input-format-listener' {$this->output["factor"]}  id='{$this->output["element_child_id"]}' name='{$this->output["element_name"]}' placeholder='{$this->data->placeholder}' type='text' value='{$this->data->value}' data-id='{$this->id}' data-settings='{$this->add_vars["data_settings"]}' {$this->output["style"]} {$this->output["required"]} {$add_attr} />{$show_price}";
		$el_text .= $el_text_add;
